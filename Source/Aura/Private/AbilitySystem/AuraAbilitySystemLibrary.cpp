...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(const FDamageEffectParams& DamageEffectParams)
{
	...
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(
		SpecHandle,
		GameplayTags.Debuff_Info_Stagger,
		DamageEffectParams.DebuffStagger);
...
TArray<FRotator> UAuraAbilitySystemLibrary::EvenlySpacedRotators(const FVector& Forward, const FVector& Axis, float Spread, int32 NumRotators)
{
	...
	if (NumRotators > 1)
	{
		...
		{
			const FVector Direction = RightOfSpread.RotateAngleAxis(-DeltaSpread * i, Axis);
...
bool UAuraAbilitySystemLibrary::IsNotFriend(AActor* FirstActor, AActor* SecondActor)
{
	...
	return bFirstIsPlayer != bSecondIsPlayer; // If not the same, then enemies
}
